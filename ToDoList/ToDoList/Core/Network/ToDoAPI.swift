//
//  ToDoAPI.swift
//  ToDoList
//
//  Created by vs on 27.08.2025.
//

import Foundation

protocol ToDoAPIProtocol {
    func fetchTasks(completion: @escaping (Result<[ToDoDTO], Error>) -> Void)
}

final class ToDoAPI {
    
    private let client: APIClientProtocol
    private let baseURL = URL(string: String.apiEndpoint)!
    
    init(client: APIClientProtocol = APIClient()) {
        self.client = client
    }
    
}

// MARK: - ToDoAPIProtocol

extension ToDoAPI: ToDoAPIProtocol {
    
    func fetchTasks(completion: @escaping (Result<[ToDoDTO], Error>) -> Void) {
        fetchTaskList { result in
            switch result {
            case .success(let value):
                completion(.success(value.todos))
            case .failure(let error):
                completion(.failure(error))
            }
        }
    }
    
    private func fetchTaskList(completion: @escaping (Result<ToDosDTO, Error>) -> Void) {
        client.request(baseURL, method: .get, completion: completion)
    }
    
}
